    ...
    local gossip table and CapacityScore (S6) LS-default selector with KMM
    provisioning in approximately $150\,ms$; and (iii) gossip-based neighbor
    detection as a fallback for simultaneous fog-node and sensor--fog channel
    failure.
...
Paillier keys, CP-ABE keys, and the storage key. After successful
...
own sealed fog-scoped key.
    ...
    \begin{equation}
    F_{target}=\arg\max_{F_j\in\mathcal{C}} CapacityScore(F_j,\,\tau_{type})
    \end{equation}
    \begin{equation}
...
\begin{algorithmic}[1]
\REQUIRE Gossip table, $T_g$, $r$, $T_{ack}$, $\tau$, aggregates $\{C_{agg,F_i}\}$, $sk_P$, $k_{store}$, $pk_{ABE}$, policy $P$
\ENSURE Cloud stores $\{C_{data},C_k\}$
...
\STATE $C_k\leftarrow Enc_{CP\text{-}ABE}(pk_{ABE},k_{store},P)$
...
\end{algorithm}
    ...
    The CapacityScore of candidate fog node $F_j$ for a task of type $\tau_{type}$ is computed as

    \begin{equation}
    \begin{aligned}
    CapacityScore(F_j,\tau_{type}) =\;& w_1(1-Workload(F_j)) \\
    &+ w_2(1-\ell_j) \\
    &+ w_3(1-q_j) \\
    &+ w_4\cdot compat(F_j,\tau_{type})
    \end{aligned}
    ...
    \begin{equation}
    \begin{aligned}
    \ell_j &= \frac{lat_j}{40.0},\\
    q_j &= \min\!\left(\frac{\sum_{\tau\in R_j} d_{\tau,cpu}}{CPU_j^{cap}},1\right)
    \end{aligned}
    \label{eq:e5-normalized-latency-queue}
    \end{equation}

    where $lat_j$ is the node latency in milliseconds, 40.0~ms is the E5 latency-normalization bound, $R_j$ is the set of running or queued tasks on node $F_j$, and $d_{\tau,cpu}$ is the CPU-core demand of task $\tau$. The compatibility term $compat(F_j,\tau_{type})\in[0,1]$ captures the structural suitability of a node for the task type. For Latency-Sensitive (LS) tasks, which are bandwidth-dominant, it weights available bandwidth heavily:
    ...
    \begin{equation}
    F_{target} = \arg\max_{F_j \in N_i} CapacityScore(F_j,\,\tau_{type})
    \end{equation}

    where $N_i$ represents the set of neighboring fog nodes discovered through the gossip-based status exchange. In the E5 implementation, $N_i$ excludes failed nodes and the primary node. Strategies S3--S6 first restrict $N_i$ to nodes with $Workload(F_j)<\tau$ and fall back to all available non-primary nodes only if no under-threshold candidate exists. Strategies S1 and S2 intentionally do not apply this workload filter, modelling blind random and round-robin routing under stress.

    The compared E5 strategy scores are:
    ...
    \begin{equation}
    \begin{aligned}
    score_{S5}(F_j,\tau_{type}) ={}& w_1(1-Workload(F_j)) \\
    &+ w_2(1-\ell_j) \\
    &+ w_3(1-q_j)
    \end{aligned}
    \label{eq:s5-score}
    ...
    \begin{equation}
    \begin{aligned}
    score_{S6}(F_j,\tau_{type}) ={}& CapacityScore(F_j,\tau_{type})
    \end{aligned}
    \label{eq:s6-score}
    \end{equation}

    S1 chooses uniformly at random from candidates, S2 uses a persistent round-robin index, S3 chooses the candidate with minimum $Workload(F_j)$, S4 uses fixed CapScore weights, S5 uses task-adaptive weights without a compatibility term, and S6 is the proposed CapacityScore with the compatibility term.
    ...
    Six strategies are compared: S1 random, S2 round-robin, S3 threshold/minimum-workload selection, S4 CapScore-Fixed, S5 CapScore-Adaptive, and S6 CapacityScore (proposed). S4 ranks candidates using workload, latency, and queue score with fixed weights $(0.50,0.15,0.35,0)$. S5 adapts the first three weights by task type but has no compatibility term. S6 adds a task-type compatibility term: LS routing prioritizes bandwidth availability, while HP routing applies a $\phi^2$ CPU-compatibility penalty so that the slowest node $F_3$ is structurally disfavored for HP tasks. Results are averaged over 20 seeds $\{42,\ldots,61\}$ with 95\% confidence intervals.
    ...
    \textbf{Results.} Fig.~\ref{fig:e6-fault} shows data loss rates and recovery latency across methods. Loss rates are reported as a fraction of one 500~ms reference window (400 readings), yielding a per-window-equivalent normalization; for outages spanning multiple windows this denominator is held fixed at 400 so that all methods are compared on a common scale. With one failed fog node serving 20 sensors (80 readings per 500~ms), loss scales with the recovery-latency window. In the worst-case scenario (failure at $t{=}0$), the proposed method (using CapacityScore S6 with LS-default weight configuration for backup node selection) loses approximately 6\% of readings per 500~ms equivalent ($\approx$24 of 400 during the 150~ms recovery window). Recovery latency is measured from the first missed ACK event, not from the failure instant; worst-case failure-to-detection can be up to one inter-reading interval (125~ms) longer if the node fails immediately after a successful transmission. The proposed method compares with 60\% for B1 gossip (1500~ms outage, equivalent to three reference windows), 20\% each for B3 checkpoint and B4 multilayer (500~ms detection windows), and approximately 30\% for B5 clustering (750~ms window). B2 replication achieves zero data loss at the cost of doubling message overhead. The proposed method incurs a message overhead of $1.055\times$ the baseline, attributable to per-reading ACK messages and two KMM provisioning messages per recovery event. Fig.~\ref{fig:e6-tradeoff} illustrates the data-loss versus message-overhead tradeoff space; the proposed method is nearest to the origin among single-mechanism approaches.
