...
\STATE Each fog node gossips $\{workload,queue,latency,\phi,compat_{LS},compat_{HP},timestamp\}$ every $T_g$
...
\end{algorithm}
    ...
    \textbf{Slot protocol correctness --- single-source (E3a).} The slot protocol was verified over 100 independent trials at delegation counts $k\in\{1,5,10,20,50\}$ under single-source delegation ($F_1\to F_4$). All trials produced zero scaled-integer arithmetic error across all values of $k$. A residual quantization deviation of $\approx$0.045 (median) is inherent to fixed-point Paillier encoding with $scale=1000$ and is consistent across all delegation counts (Fig.~\ref{fig:e3a-correctness}).

    \begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\linewidth]{../figures/exp3a_correctness.pdf}
    \caption{E3a slot protocol correctness under single-source delegation ($F_1\to F_4$) over 100 trials at five delegation counts $k$. All trials achieve zero scaled-integer error; the residual is fixed-point quantization noise.}
    \label{fig:e3a-correctness}
    \end{figure}

    \textbf{Slot protocol correctness --- multi-source (E3b).} The protocol was further verified under simultaneous dual-source delegation ($F_1,F_2\to F_4$), with each source delegating 5 readings from a pool of 30 and the backup node holding 30 own readings (90 sensors total). Over 100 trials, all produced zero scaled-integer arithmetic error. The median quantization deviation was $\approx$0.041, consistent with E3a, confirming that zero-fill correctness extends to concurrent multi-source delegation (Fig.~\ref{fig:e3b-multisource}).

    \begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\linewidth]{../figures/exp3b_multisource.pdf}
    \caption{E3b slot protocol correctness under simultaneous dual-source delegation ($F_1,F_2\to F_4$) over 100 trials. Zero scaled-integer error is maintained across all trials; quantization deviation is consistent with E3a.}
    \label{fig:e3b-multisource}
    \end{figure}

    \textbf{KMM combine overhead (E4).} The KMM window-combine latency was measured at $k\in\{1,2,5,10,20,50,100\}$ fog aggregates. As shown in Fig.~\ref{fig:e4-kmm}, the combine latency remains constant at $1.474$~ms across all values of $k$, confirming that the KMM combine step is negligible relative to the 500~ms aggregation window even as the number of fog nodes scales to 100. This is consistent with the E2 pipeline breakdown, where the KMM combine contributes only $1.474$~ms of the total $444.128$~ms.

    \begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\linewidth]{../figures/exp4_kmm_combine.pdf}
    \caption{E4 KMM window-combine latency vs number of fog aggregates $k\in\{1,\ldots,100\}$. Latency is constant at $1.474$~ms for all $k$, confirming that the combine step does not become a bottleneck as the deployment scales.}
    \label{fig:e4-kmm}
    \end{figure}
    ...
    Experimental evaluation across eight experiments demonstrated: constant per-window cloud storage independent of sensor count (E1); aggregate correctness under mid-window delegation in all 100 trials for both single-source and multi-source delegation (E3a--E3b); constant KMM combine latency of $1.474$~ms across up to 100 fog aggregates (E4); the highest deadline satisfaction rate of 78.77\% and the lowest bandwidth-utilization variance among six compared strategies (E5); worst-case data loss reduced from 75\% under gossip-only detection to 7.5\% with the ACK+KMM mechanism at only $1.055\times$ message overhead (E6); and 60--80\% reduction in key-exposure blast radius relative to a global-key design under single-node compromise (E8).
