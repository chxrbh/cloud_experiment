...
The proposed framework replaces the PRE delegation layer with a Trusted Execution Environment (TEE) channel implemented via Intel SGX enclaves and a Key Management Module. This eliminates the need for the intermediate node to hold re-encryption key material in host memory, bounding the blast radius of fog node compromise to a single fog-scoped key group. This design follows the direction identified by recent surveys on secure fog computing, which emphasise hardware-rooted trust as the emerging approach for overcoming the limitations of software-only delegation schemes~\cite{nemati2024,kumar2023}.
    ...
    The process includes sensor-level AES-GCM encryption, fog-node tag verification, SGX-protected AES-to-Paillier conversion, Paillier slot aggregation, per-reading TEE delegation, KMM window combine, and secure AES-GCM cloud storage.
...
own sealed fog-scoped key.
    ...
    Each sensor encrypts its readings using AES-128-GCM under the fog-scoped key of its assigned fog node (four readings per window as stated in Section~\ref{sec:arch}). A sensor assigned to Fog~A encrypts only with $k_{fogA}$. Upon receiving a ciphertext, the fog node verifies the AES-GCM authentication tag using its provisioned fog-scoped key before passing the accepted ciphertext to the SGX enclave for processing. A single fog-node failure therefore affects 80 of 400 readings (20\% of window traffic) under an even sensor distribution.
...
\end{algorithm}
    ...
    \textbf{Aggregation latency (E2).} Using calibrated 2048-bit Paillier timings, the proposed pipeline requires $444.128$~ms in total: $227.537$~ms for fog TA conversion, $1.474$~ms for host aggregation and KMM window combine, and $215.117$~ms for storage TA preparation (Fig.~\ref{fig:e2-latency}). The total is within the 500~ms aggregation window; fog conversion and storage preparation dominate, while the KMM combine step contributes only $1.474$~ms regardless of fog-node count (tested up to $k=100$ aggregates). These timings are host-reference values obtained from the OP-TEE QEMU simulation described in Section~\ref{sec:eval-setup} and should not be interpreted as physical hardware timings.

    Under hardware-target assumptions (AES-NI, hardware Paillier co-processor), an analytical model places the proposed pipeline at 399~ms in non-delegation windows and 549~ms in delegation windows, where the additional 150~ms covers TEE key provisioning. The 500~ms budget is satisfied in 12 of 20 simulated windows; delegation burst phases (W6--10, W18--20) exceed it by 150~ms. Deployments should account for this overhead through capacity headroom or an extended window during burst phases. Fig.~\ref{fig:e7-pipeline} shows per-window latency across the 20-window schedule.
    ...
    \caption{Analytical pipeline latency (left) and per-window storage items (right) across 20 windows under hardware-target constants. The dashed line marks 500~ms; delegation windows exceed the budget by 150~ms.}
    \label{fig:e7-pipeline}
    \end{figure}

    \textbf{Slot protocol correctness (E3).} The slot protocol was verified over 100 independent trials at delegation counts $k\in\{1,5,10,20,50\}$ under single-source delegation ($F_1\to F_4$) and simultaneous dual-source delegation ($F_1,F_2\to F_4$). All trials produced zero scaled-integer arithmetic error. A residual quantization deviation of $\approx$0.045 (median) is inherent to fixed-point Paillier encoding with $scale=1000$ and is consistent across all delegation counts and configurations (Fig.~\ref{fig:e3-correctness}).
    ...
    B3 & Checkpoint~\cite{shahab2025sfc}: 500~ms checkpoint interval; readings after latest checkpoint lost until restore. \\
