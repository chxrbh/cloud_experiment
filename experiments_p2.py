...
def run_e4(
    ...
    rows = []
    ct_bytes = CALIBRATION_PAILLIER_CIPHERTEXT_BYTES
    for k in k_values:
        if k < 1:
        ...
        rows.append(
            {
                "k_fog_aggregates": k,
                "he_additions": k - 1,
                "combine_latency_ms_median": CALIBRATION_HOST_AGGREGATE_MEAN_MS,
                "combine_latency_ms_std": 0.0,
                "bytes_received": k * ct_bytes,
                "ciphertext_bytes": ct_bytes,
                "within_500ms": CALIBRATION_HOST_AGGREGATE_MEAN_MS <= WINDOW_MS,
                "benchmark_mode": CALIBRATION_BENCHMARK_MODE,
                "paillier_key_bits": CALIBRATION_PAILLIER_KEY_BITS,
