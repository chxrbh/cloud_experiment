...
def _finish_figure(_results_dir: str, basename: str, show: bool) -> None:
    os.makedirs(FIGURES_DIR, exist_ok=True)
    plt.savefig(os.path.join(FIGURES_DIR, f"{basename}.png"))
    plt.savefig(os.path.join(FIGURES_DIR, f"{basename}.pdf"))
    if show:
        plt.show()
    else:
        plt.close()
...
def plot_e2(results_dir: str = RESULTS_DIR, show: bool = False) -> None:
    ...
    ax.set_title("E2a - Calibrated latency vs n")
    ...
    fig.suptitle("E2 - Aggregation Latency: Calibrated 2048-bit Reference", fontweight="bold", y=1.01)
    plt.tight_layout()
...
def plot_e5(results_dir: str = RESULTS_DIR, show: bool = False) -> None:
    rows = read_csv(os.path.join(results_dir, E5_AGGREGATE_CSV))
    rows = sorted(rows, key=lambda row: E5_STRATEGY_NAMES.index(row["strategy"]))
    labels = [E5_STRATEGY_LABELS[row["strategy"]].replace(": ", "\n") for row in rows]
    bar_colors = [E5_COLORS[row["strategy"]] for row in rows]
    fig, axes = plt.subplots(2, 2, figsize=(13, 9))

    def annotate(ax, values: list[float], lower_better: bool = False, pct: bool = False) -> None:
        ...
        for i, (bar, val) in enumerate(zip(ax.patches, values)):
            ...
            text = f"{val * 100:.1f}" if pct else f"{val:.3f}"
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + max(values) * 0.01, text, ha="center", va="bottom", fontsize=8.5, fontweight=weight)

    ax = axes[0, 0]
    values = [_float(row, "deadline_rate_mean") for row in rows]
    ax.bar(labels, values, color=bar_colors, width=0.5, edgecolor="white")
    annotate(ax, values, pct=True)
    ...
    ax = axes[0, 1]
    values = [_float(row, "completion_rate_mean") for row in rows]
    ax.bar(labels, values, color=bar_colors, width=0.5, edgecolor="white")
    annotate(ax, values, pct=True)
    ...
    ax = axes[1, 0]
    values = [_float(row, "sigma_bw_mean_mean") for row in rows]
    ax.bar(labels, values, color=bar_colors, width=0.5, edgecolor="white")
    annotate(ax, values, lower_better=True)
    ax.set_title("Bandwidth utilization std dev\n(lower = better)")
    ax.set_ylabel("sigma BW")
    ax.tick_params(axis="x", labelrotation=12)

    ax = axes[1, 1]
    values = [_float(row, "t_wait_mean_mean") for row in rows]
    ax.bar(labels, values, color=bar_colors, width=0.5, edgecolor="white")
    annotate(ax, values, lower_better=True)
    ax.set_title("Mean waiting time\n(lower = better)")
    ax.set_ylabel("ms")
    ax.tick_params(axis="x", labelrotation=12)
    ...
    fig.suptitle("E5 - Full-CapScore vs Six Delegation Strategies\nTask-level simulation across 20 fixed seeds", fontweight="bold", y=1.02)
    plt.tight_layout()
    _finish_figure(results_dir, "e5_results", show)


def plot_e5_task_quality(results_dir: str = RESULTS_DIR, show: bool = False) -> None:
    rows = read_csv(os.path.join(results_dir, E5_TASK_RECORDS_CSV))
    fig, axes = plt.subplots(1, 2, figsize=(12, 4.5))
    strategies = E5_STRATEGY_NAMES
    colors = [E5_COLORS[s] for s in strategies]

    hp_miss = []
    hp_f3 = []
    for strategy in strategies:
        hp_rows = [row for row in rows if row["strategy"] == strategy and row["task_type"] == "HP"]
        hp_miss.append(1.0 - sum(int(row["deadline_met"]) for row in hp_rows) / len(hp_rows))
        hp_f3.append(sum(1 for row in hp_rows if row["assigned_node"] == "F3") / len(hp_rows))

    labels = [E5_STRATEGY_LABELS[s].replace(": ", "\n") for s in strategies]
    x = np.arange(len(strategies))
    axes[0].bar(labels, hp_miss, color=colors, width=0.55, edgecolor="white")
    axes[0].set_ylabel("HP deadline miss rate")
    axes[0].set_title("HP task deadline misses\n(lower = better)")
    axes[0].yaxis.set_major_formatter(plt.FuncFormatter(lambda v, _: f"{v:.0%}"))

    axes[1].bar(labels, hp_f3, color=colors, width=0.55, edgecolor="white")
    axes[1].set_ylabel("HP assignment rate to F3")
    axes[1].set_title("HP routing to slow node F3\n(lower = better)")
    ...
    plt.tight_layout()
    _finish_figure(results_dir, "e5_task_quality", show)
...
def generate_all(results_dir: str = RESULTS_DIR, show: bool = False) -> None:
    ...
    plot_e5_task_quality(results_dir, show=show)
