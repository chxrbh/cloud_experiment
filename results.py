...
def write_summary(
    ...
    conclusion = (
        "At least one calibrated E2 row exceeds the 500 ms aggregation window."
    ...
    with open(path, "w", encoding="utf-8") as fh:
        ...
        fh.write(
            f"Interpretation: E2 uses the calibrated 2048-bit host-reference benchmark for the proposed "
            f"path. The calibrated mean total is {CALIBRATION_E2_TOTAL_MS:.3f} ms, within the 500 ms "
            f"window. Caveat: {CALIBRATION_NOTE}\n\n"
        )
        fh.write(
...
def write_p2_summary(
    ...
    with open(path, "w", encoding="utf-8") as fh:
        ...
        fh.write(
            f"E4 note: KMM combine uses the calibrated 2048-bit host aggregate reference "
            f"({CALIBRATION_HOST_AGGREGATE_MEAN_MS:.3f} ms). Caveat: {CALIBRATION_NOTE}\n\n"
        )
        fh.write(
